...
**TDD mandate:** No UI component, screen, CSS file, or HTML structure may be created before a failing test for it exists. Every screen and component listed in this document has a corresponding TEST task in `tasks.MD`. The test must be written first, confirmed to fail, and then the component is implemented to make it pass. See `configuration.MD` §3 for the full TDD rules.
...
| **Share link blue** | `#1e40af` | Copy share link button (casino-style bluish accent). Light: `#1e40af`; dark: `#3b82f6`. |
...
| **Share link (copy button)** | `#1e40af` (share link blue) | `#3b82f6` |
...
**TDD for components:** Every component listed below must have a test written before the component file is created. The test must cover: renders without crashing, renders with required props, interactive states (loading, error, disabled), and any behavior described in the spec. Use React Testing Library and `@testing-library/user-event`.
...
- **Voting card** — Poker-inspired: surface color with subtle border; **selected state** uses a brighter (more luminous) appearance and a more contrasting border to clearly indicate the current selection; value (number or symbol) centered, large and readable. Before reveal: show only value; after reveal: optional description below value (if deck has descriptions). After reveal, voting cards are read-only; no selection or deselection is possible.
...
- **Global messages** — Stacked at top of viewport; visible on every page. Each message: short text, close button on right. Types: info, error, success (distinct border/background). Messages clear when user navigates to another route.
...
**TDD for screens:** Every screen below has a paired TEST task in `tasks.MD`. Before creating any screen file, write a React Testing Library test that covers all states listed for that screen (empty, loading, error, success, role-based variants). The test must fail first. Only then implement the screen to make the test pass.
...
- **Header:** Room name; badge (Open/Closed); [Menu] or [Leave] for participant, [Close room] [Leave] [Copy share link] for facilitator. Copy share link button to the right of Leave; bluish casino-style color (`--color-share-link`).
...
**States:** No item; voting (secret); revealed; finalized (next item or end). Room closed: 5-second countdown with "Go now" button; redirect to Create Room; global message "The room [name] was closed at [HH:mm:ss]".
...
- **Purpose:** Add or edit estimation item (title, description). Used before or during voting (until reveal).
- **Who:** Facilitator only.
- **Layout:** Modal or inline form. Title “Add item” / “Edit item”; fields: “Title” (required), “Description” (optional); “Save” and “Cancel”. If edit: “Remove item” (destructive, with confirmation; only when no votes cast).
...
- **Keyboard:** Enter to submit (save); Shift+Enter in description adds newline.
...
### 7.10 Estimation Process — Voting and Reveal (User Story 6)

- **Purpose:** Implement the full estimation flow: add item, secret voting, reveal, re-vote, record final estimate, post-reveal statistics.
- **Who:** Facilitator (add item, reveal, re-vote, record final); participant and facilitator (vote); observer (view only after reveal).
- **Card selection and reveal rules:**
  - **Before reveal:** Everyone can select, deselect, or change their card. Single tap to select; tap same card again to deselect; tap different card to change. Selected card has distinct visual feedback (brighter, more contrasting border).
  - **After reveal:** No one can select, change, or deselect. Voting controls are hidden; votes are read-only.
- **Voting (6.2):** Cards show values only (no descriptions before reveal). Facilitator can vote like participants. Feedback: "X of Y have voted".
- **Reveal (6.3):** All votes displayed simultaneously. Card descriptions (if any) become visible. Highlight highest and lowest.
- **Post-reveal statistics (6.6):** Show average value, suggested estimate (rounded from average), vote distribution (e.g. "3 voted 5, 2 voted 8, 1 voted 13").
- **Re-vote (6.4):** "Re-vote" clears current round; previous round kept in history.
- **Record final (6.5):** Facilitator selects or confirms value; "Use suggested" option. Item marked estimated; next item or close.
- **Responsive:** Cards and buttons 44 px min; keyboard for card selection on desktop.
...
| `drivin-design/configuration.MD` | Workflow, TDD rules, and AI rules. |
| `drivin-design/quickstart.MD` | Stack (React, Expo, shared), testing tools, and commands. |
| `drivin-design/tasks.MD` | Implementation tasks with paired TEST tasks for every screen and component. |
| `drivin-design/ui-definition.MD` | This file — UI rules, themes, components, screens. |

When implementing UI, use this document for layout, components, themes, and input behavior; use `spec.MD` for what the screen must do (success criteria); use `tasks.MD` to find the TEST task that must be completed first.
